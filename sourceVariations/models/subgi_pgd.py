    ...
    def forward(self, nf, emb, features):
        ...
        if self.model_id == 0:
            ...
            for i in range(nf.num_blocks)[::-1]:
                # 1) Map parent (global) node IDs in this layer to local IDs of the subgraph
                parent_ids = nf.layer_parent_nid(i + 1).cpu().long()
                v = small_g.map_to_subgraph_nid(parent_ids)
                # DGL may return a tuple; take the IDs tensor
                if isinstance(v, tuple):
                    v = v[0]
                # 2) Filter out nodes that are not in the subgraph (marked as -1)
                mask = v >= 0
                if mask.sum().item() == 0:
                    # Keep list lengths aligned: append an empty score tensor for this block
                    edge_embs.append(torch.empty(0, 1, device=emb.device))
                    continue
                v = v[mask]

                # 3) Collect outgoing edge IDs for those local nodes
                uid = small_g.out_edges(v, 'eid')
                if uid.numel() == 0:
                    edge_embs.append(torch.empty(0, 1, device=emb.device))
                    continue

                # 4) Run the discriminator layer with correct depth flag
                depth = 1 if (i + 1 == nf.num_blocks) else 2
                h = self.dc_layers[0](small_g, v, uid, depth)
