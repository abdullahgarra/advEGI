    ...
    for runs in tqdm(range(100)):
        ...
        for epoch in range(args.n_dgi_epochs):
            ...
            if loss < best:
                ...
                best_t = epoch
                cnt_wait = 0
                torch.save(dgi.state_dict(), 'best_classification_USA_{}.pkl'.format(args.model_type))
            else:
                cnt_wait += 1
        ...
        #     pass
        # else:
        dgi.load_state_dict(torch.load('best_classification_USA_{}.pkl'.format(args.model_type)))
        ...
        with torch.no_grad():
            if args.model_type == 1:
                ...
                # # Structure-aware eval: pass NodeFlow (ego-graphs) to the encoder
                # test_sampler = dgl.contrib.sampling.NeighborSampler(
                #     g,
                #     g.number_of_nodes(),   # seed all nodes so we get one big NF
                #     -1,                    # sample all neighbors within num_hops
                #     neighbor_type='in',
                #     num_workers=1,
                #     add_self_loop=False,
                ...
                #     nf.copy_from_parent()  # copies g.ndata['features'] into nf
                #     # Try common SubGI encoder signatures
                #     try:
                #         embeds = dgi.encoder(features, nf)        # (nf, corrupt=False)
                #     except TypeError:
                #         print("error bro")
                #         break  # only one NF because we seeded all nodes
